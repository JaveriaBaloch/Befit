...
function Dietitian(){
    ...
    useEffect(() => {
        axios.post("http://localhost:3001/Dietitian",{email:localStorage.getItem("email")})
...
const deleteQuery =(id) =>{
        ...
        .then(()=>axios.post("http://localhost:3001/Dietitian",{email:localStorage.getItem("email")}).then(res=>setData(res.data.queries))
        )
        .catch()
        console.log(id)
    }
    ...
    return(
        <section className="row mb-5" id="Contact">
            ...
            <div className="heading mx-auto mb-5">
                    <h1 className="text-center mt-3 text-black">Dietitian</h1>
